"""Legacy weekly OR-Tools CP-SAT solver for official PS1 Scenarios A, B, and C.

Preserved for benchmark comparisons, regression testing, and emergency debugging.
New development uses the date-first daily-integrated solver in daily_solver.py.
"""
...
def _build_weekly_model(
    problem: ProblemInstance,
    scenario: Scenario,
) -> _WeeklyModelArtifacts:
    """Build all common constraints and the selected scenario policy."""

    model = cp_model.CpModel()
    footprints = FootprintCache(problem)
    artifacts = _WeeklyModelArtifacts(model=model, scenario=scenario, footprints=footprints)
...
def _extract_weekly_rows(
    problem: ProblemInstance,
    artifacts: _WeeklyModelArtifacts,
    solver: cp_model.CpSolver,
) -> tuple[
...
def solve_weekly_ps1(
    problem: ProblemInstance,
    scenario: Scenario | str,
    options: PS1SolveOptions | None = None,
) -> PS1SolveResult:
    """Solve one PS1 scenario using the legacy weekly formulation."""

    selected_scenario = coerce_scenario(scenario)
    solve_options = options or PS1SolveOptions()
    started = monotonic()
    artifacts = _build_weekly_model(problem, selected_scenario)
    ...
    access_rows, occupancy_rows, selected_windows = _extract_weekly_rows(
...
solve_legacy_weekly_ps1 = solve_weekly_ps1
