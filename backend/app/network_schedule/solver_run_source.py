...
class SolverRunScheduleSource(NetworkScheduleSource):
    ...
    def get_available_scenarios(self) -> list[ScenarioAvailability]:
        with self.session_factory.session() as session:
            stmt = select(SolverRun.scenario).where(SolverRun.status == "SUCCESS")
            if self.revision_id:
                stmt = stmt.where(SolverRun.revision_id == self.revision_id)
            available = set(session.scalars(stmt).all())
            return [
    ...
    def get_accesses(
        ...
        with self.session_factory.session() as session:
            run_stmt = select(SolverRun.id).where(
                SolverRun.scenario == scenario,
                SolverRun.status == "SUCCESS",
            ).order_by(SolverRun.created_at.desc())
            if self.revision_id:
                run_stmt = run_stmt.where(SolverRun.revision_id == self.revision_id)
            run_id = session.scalar(run_stmt.limit(1))
            if not run_id:
                return []

            stmt = select(RunAccess).where(RunAccess.run_id == run_id)
            if week is not None:
                stmt = stmt.where(RunAccess.week == week)
            if activity_id is not None:
                stmt = stmt.where(RunAccess.activity_id == activity_id)
    ...
    def get_occupancies(
        ...
        with self.session_factory.session() as session:
            ...
            stmt = select(RunOccupancy).where(RunOccupancy.run_id == run_id)
            if week is not None:
                stmt = stmt.where(RunOccupancy.week == week)
            if activity_id is not None:
                stmt = stmt.where(RunOccupancy.activity_id == activity_id)
            rows = session.scalars(stmt).all()
            return [
                OccupancyScheduleRow(
                    activity_id=r.activity_id,
    ...
    def get_results(self, scenario: str) -> list[ScenarioResultRow]:
        with self.session_factory.session() as session:
            ...
            stmt = select(RunContractResult).where(
                RunContractResult.run_id == run_id,
                RunContractResult.scenario == scenario,
            )
            rows = session.scalars(stmt).all()
            return [
                ScenarioResultRow(
