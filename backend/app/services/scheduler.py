...
def solve(snapshot, engine="cp_sat", time_limit=8):
    baseline_errors = check_plan(snapshot, snapshot["committed_allocations"])
    if baseline_errors:
    ...
    result = (cp_sat if engine == "cp_sat" else demo_search).solve(snapshot, time_limit)
