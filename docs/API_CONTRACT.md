...
| Implemented | `GET /api/ps1/calendar-preview/context` | Restore preview setup | Authenticated, read-only latest imported source, matching instance/calendar and latest/last complete attempt IDs. Optional `bundle_id` or `scenario` (`A`, `B`, `C`) filters context by specific source. Never imports sample files or starts a solve. |
| Implemented | `POST /api/ps1/calendar-preview/assign-default` | Assign actual nights automatically | Officer-only zero-body endpoint; reads configured solver outputs (`/outputs/{scenario}`, defaulting to `A`), binds to seeded official instance revision, validates fixed weekly schedule, creates/reuses assumed demo calendar and queues CP-SAT date assignment. Optional query parameter `?scenario=A|B|C`. |
