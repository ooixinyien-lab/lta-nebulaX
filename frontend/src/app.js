...
import { officerPage, resourcesPage } from './pages/officer.js?v=20260910q';
...
function seedManualPlan(force=false) {
  ...
  state.manualPlan=state.snapshot.committed_allocations.map(a=>({...a,locked:!!a.locked}));
...
function render() {
  ...
  const nav=officer?[['planner','Planning workspace','<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 3v3M17 3v3M4 9h16M5 5h14a1 1 0 0 1 1 1v13H4V6a1 1 0 0 1 1-1Zm3 8h3v3H8Z"/></svg>'],['resources','Resources & activity','<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3 2"/></svg>']]:[['my-requests','My requests','<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 4h14v16H5zM8 9h8M8 13h8M8 17h5"/></svg>'],['new-request','New request','<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg>']];
  ...
  root.innerHTML=`<div class="app-shell"><aside class="sidebar ${state.navExpanded?'nav-expanded':''}"><a class="brand" href="#"><span class="brand-mark">R</span><span class="brand-label">RailPlan</span></a><div class="sidebar-kicker">MAINTENANCE WORKSPACE</div><nav>${nav.map(([id,label,icon])=>`<button class="nav-item ${state.page===id?'active':''}" data-nav="${id}" title="${h(label)}"><span class="nav-icon">${icon}</span><span class="nav-label">${h(label)}</span></button>`).join('')}</nav><div class="sidebar-note"><span class="status-dot"></span><strong>Fictional corridor</strong><p>Planning support only.<br>No live track access.</p></div><div class="sidebar-user"><div class="avatar">${officer?'O':'R'}</div><div class="user-copy"><strong>${h(state.user.name)}</strong><small>${officer?'Planning officer':'Requester'}</small></div><button data-action="logout" title="Sign out" aria-label="Sign out">&rarr;</button></div></aside><div class="app-main"><header class="topbar"><span>NEBULA / Track 1 <span class="crumb">/ Team starter</span></span><div>${badge('SYNTHETIC DATA','neutral')}${badge(state.config.auth_mode==='demo'?'LOCAL DEMO':'SUPABASE AUTH',state.config.auth_mode==='demo'?'warning':'success')}</div></header><main class="content ${planner?'planner-content':''}"><div class="page-heading"><div><div class="eyebrow dark">${officer?'OFFICER CONSOLE':'REQUESTER PORTAL'}</div><h1>${title}</h1>${subtitle?`<p>${subtitle}</p>`:''}</div><div class="page-actions">${showDatePicker?`<select id="date-picker" aria-label="Planning date">${dates.map(d=>`<option value="${d}" ${state.date===d?'selected':''}>${d}</option>`).join('')}</select>`:''}<button class="button secondary" data-action="refresh" aria-label="Refresh">&#8635;</button>${plannerActions}</div></div><div id="notice" role="alert"></div>${content}<footer class="app-footer"><span>Revision ${state.snapshot.metadata.planning_version} &middot; ${state.config.solver_engine==='cp_sat'?'CP-SAT configured':'Tiny-demo search / not CP-SAT'}</span><span>All constraints are prototype assumptions.</span>${officer&&state.config.auth_mode==='demo'?'<button class="text-button" data-action="reset">Reset synthetic demo</button>':''}</footer></main></div></div>`;
...
document.addEventListener('click',event=>{
  ...
  if(el.dataset.nav) { if(state.page===el.dataset.nav)return;state.page=el.dataset.nav;render();return; }
  ...
  if(el.dataset.action==='add-to-schedule')return run(async()=>{const previousPlan=clonePlan(),scheduled=new Set(state.manualPlan.map(a=>a.request_id)),selected=state.selectedPool.size?state.selectedPool:null,candidates=state.snapshot.requests.filter(r=>['approved','scheduled'].includes(r.status)&&!scheduled.has(r.id)&&(!selected||selected.has(r.id)));if(!candidates.length)return 'Select or approve at least one request first.';for(const request of candidates){const allocation={request_id:request.id,start:request.preferred_start,end:endAt(request.preferred_start,duration(request)),engineer_id:request.preferred_engineer||request.eligible_engineers[0],equipment_ids:[...request.required_equipment_ids],locked:false};state.manualPlan.push(allocation);const best=bestStartOnDate(request.id,allocationDate(allocation.start));allocation.start=best;allocation.end=endAt(best,duration(request));}state.selectedPool.clear();state.proposal=null;const result=await checkManualPlan();recordPlanHistory(previousPlan);const count=conflictCount(result.issues);return `${candidates.length} approved job${candidates.length===1?'':'s'} added without moving existing work${count?`; ${count} conflict${count===1?'':'s'} need attention`:''}.`;});
  ...
  if(el.dataset.action==='generate') return run(async()=>{const previousPlan=clonePlan(),unlocked=state.manualPlan.filter(a=>!a.locked);for(const allocation of unlocked){const request=state.snapshot.requests.find(r=>r.id===allocation.request_id),best=bestStartOnDate(allocation.request_id,allocationDate(allocation.start));allocation.start=best;allocation.end=endAt(best,duration(request));}state.proposal=null;const result=await checkManualPlan();recordPlanHistory(previousPlan);const count=conflictCount(result.issues);return `Auto-scheduled ${unlocked.length} existing job${unlocked.length===1?'':'s'}${count?`; ${count} conflict${count===1?'':'s'} need attention`:'.'}`; });
