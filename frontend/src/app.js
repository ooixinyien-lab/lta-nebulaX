...
document.addEventListener('click',event=>{
  ...
  if(el.dataset.action==='generate'&&state.config.ui_demo) return run(async()=>{const previous=clonePlan();const p=await api('/schedule/proposals',{method:'POST',body:{locked_allocations:draftPayload().filter(a=>a.locked)}});state.manualPlan=p.allocations;state.proposal=p;await checkManualPlan();recordPlanHistory(previous);return 'Preset demo schedule restored; locked jobs kept in place.';});
