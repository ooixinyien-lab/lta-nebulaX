...
class NetworkMapService:
    ...
    def __init__(
        ...
        self.schedule_source = schedule_source or get_schedule_source()
    ...
    def get_context(self) -> dict[str, Any]:
        ...
        with self.db_session_factory.session() as session:
            rev = session.scalar(
                ...
                .order_by(C(InstanceRevision).created_at.desc(), C(InstanceRevision).revision_number.desc())
                .limit(1)
            )
            ...
            if rev:
                params = session.scalars(
                    select(ParameterRow).where(C(ParameterRow).revision_id == rev.id)
                ).all()
                for p in params:
    ...
    def get_topology(self) -> dict[str, Any]:
        ...
        with self.db_session_factory.session() as session:
            ...
            line_rows = session.scalars(
                select(LineRow).where(C(LineRow).revision_id == rid) if rid else select(LineRow)
            ).all()
            stn_rows = session.scalars(
                select(StationRow).where(C(StationRow).revision_id == rid).order_by(C(StationRow).seq) if rid else select(StationRow).order_by(C(StationRow).seq)
            ).all()
            sec_rows = session.scalars(
                select(SectorRow).where(C(SectorRow).revision_id == rid).order_by(C(SectorRow).seq) if rid else select(SectorRow).order_by(C(SectorRow).seq)
            ).all()
            loc_rows = session.scalars(
                select(LocationRow).where(C(LocationRow).revision_id == rid) if rid else select(LocationRow)
            ).all()
